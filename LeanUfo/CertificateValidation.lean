...
the rebuilt module. Git metadata and SHA-256 digests are added by the exporter,
so the command-line wrapper verifies those values separately.
-/
def compareRebuiltManifest (json : Json) (rebuilt : CertificateManifest) : Except String Unit := do
  ...
  if rebuilt.fields.size != rows.size then
    throw s!"rebuilt module has {rebuilt.fields.size} certificate entries; manifest has {rows.size}"
