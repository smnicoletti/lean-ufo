...
/-!
# Positive relator model: section 3.10

The model satisfies the corrected §3.10 package with its part-based (a73). It
also satisfies the guarded-overlap formula retained for comparison. Its relator
has exactly two qua-individual proper parts. They inhere in distinct bearers and
share one perdurant foundation.
-/

namespace Relator.Model3_10
...
/-- The positive relator model satisfies the active corrected §3.10 package. -/
instance activeAxioms : UFOAxioms3_10 sig where
  toUFOAxioms3_9 := baseAxioms.toUFOAxioms3_9
  ax69 := ax69_sig
  ax70 := ax70_sig
  ax71 := ax71_sig
  ax72 := ax72_sig
  ax73 := by
    simpa [ax_a73_part_characterization] using ax73_part_sig
  ax74 := ax74_sig
  ax75 := ax75_sig
  ax76 := ax76_sig
  ax77 := ax77_sig
...
/-- The positive relator witness refutes the printed overlap-based (a73). -/
theorem not_ax73_printed : ¬ ax_a73_printed sig := by
  intro hPrinted
  exact no_relator_with_quaIndividual_properPart (Sig := sig)
    Model3_5.ax47_sig Model3_5.ax50_sig Model3_5.ax52_sig
    Model3_3.ax41_sig Model3_3.ax42_sig ax70_sig hPrinted ax74_sig
    .relator .quaA .actual
    positive_relator_witness.1
    positive_relator_witness.2.1
    (by change quaIndividual .quaA; trivial)

end Relator.Model3_10
