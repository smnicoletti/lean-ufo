...
current ax73, a relator cannot have an ordinary proper part that is a qua
...
qua-individual proper part refutes the current ax73.
-/
theorem relator_composition_refutes_current_ax73
  ...
  ∀ (r q : Sig.Thing) (w : Sig.F.World),
    Sig.Relator r w →
      Sig.ProperPart q r w →
        Sig.QuaIndividual q w → ¬ ax_a73 Sig :=
by
...
The part-based characterization is the selected repair. The guarded-overlap
characterization is retained as historical evidence for the comparison reported
in the paper. Neither formula is part of `UFOAxioms3_10` in this branch.
-/

/--
Selected analysis-only ax73 repair. It characterizes a qua individual by its
parts rather than by every entity that overlaps it.
-/
def ax_a73_part_characterization : Prop :=
...
Keeping the shared background separate prevents the analysis models from
acquiring the current ax73 through `UFOAxioms3_10` by inference.
-/
...
recorded in `RelatorRepair.GuardedOverlapCountermodel`.
-/
...
The assumptions are explicit to ensure this historical result depends on the
guarded proposition rather than the original `ax_a79` or `UFOAxioms3_10`.
-/
