...
follows from the chosen S5 semantics and the stated axioms. Kind stability
follows from numbered axioms through §3.4.
...
```lean
consistent_3_1 :
  ∃ (Sig : UFOSignature3_1.{0}), UFOAxioms3_1 Sig

consistent_4 :
  ∃ (Sig : UFOSignature4.{0}), UFOAxioms4 Sig
```
...
For (a99), `ProductSubsetOf.embedding` maps domain members injectively into
the Cartesian product of the component sets. Its premises require coordinate
membership and equality of members whenever all their coordinates agree.
Both certification and diagnostics check these conditions for each supplied
family. A zero-coordinate product permits at most one domain member.

The core axiom quantifies over arbitrary product-family
...
well-formed row for each of the 113 certificate fields. The default validation
...
semantic axiom. The complete 113-check registry theorem subsumes these slice
...
The non-sortal upward-closure check between axioms 33 and 34 has the bound
`T * (T * (32*W + 2) + 2)`. Instance-to-endurant and subtype-of-kind typing
follow from numbered axioms in §3.4 and need no separate registry entries.
...
fixed registry contains 113 delayed `BoundedCheck` entries, each pairing the
actual checker with its independently proved operational bound. Lean proves:

```lean
checkAxioms4BoundedRegistry_size :
  (checkAxioms4BoundedRegistry M).size = 113
...
sum of the 113 concrete per-check formulas plus three units per visited
...
and visited branches. On the compiled cached model, 109 registry checks have
...
all 113 entries. Direct unary, binary,
